...
\vspace{0.55ex}
\doublespacing
In this document, I present the results of one comparison study in which I reproduce Peter Diener's simulation~\cite{heffernan_ottewil_wardell_modesum_basisForCode} of a scalar particle on a Schwarzschild background for the purpose of producing Extreme Mass Ratio black hole binary templates for the Laser Interferometer Space Antenna. Porting this Fortran code to C++ has provided an independent verification of the code to roundoff precision. The results do not verify, but are consistent with, Wardell et al.'s circular orbit results from 2012 using a different differencing scheme~\cite{wardell_vega_thornberg_diener}. I present preliminary results from a second comparison study of Niels Warburton's and Peter Diener's and Barry Wardell's scalar self-force simulation for eccentric orbits. These results are so far consistent with the physical reasoning gained from Field, Hesthaven, and Lau~\cite{time_dependent_coordinate_transform}; with the caveat that their results are for the gravitational case while ours are for the scalar case. Finally, I make use of a first order Richardson extrapolation to obtain the self-force at infinite Discontinuous Galkerin order then sum the spherical harmonic modes to obtain the total self-force as a function of time. The long term goal is to continue the second comparison study with self-consistent evolution. 
\end{abstract}
